...
Original file is located at
    https://colab.research.google.com/drive/1d_k6H4AmJhrDTwhX1pzzyPMRBg-HOh4J
...
from Context_Sensitivity.utils import InputExample, convert_examples_to_features
...
class BERT_MLP():
    ...
    def __init__(self,
        ...
        self.epochs = epochs
        self.loss = loss
        self.earlystop = tf.keras.callbacks.EarlyStopping(monitor='val_MAE',
    ...
    def build(self, bias=0):
        ...
        pred = tf.keras.layers.Dense(1, activation=None, bias_initializer=tf.keras.initializers.Constant(bias))(dense)
...
"""# CA BERT (concatenate  the CLS tokens of parent and target)"""
...
"""# Encode parent with a BILSTM"""

#BERT MLP FOR REGRESSION
class CA_BILSTM_BERT(BERT_MLP):

    def __init__(self, max_length=128, word_embedding_size=100, **kwargs):
        super(CA_BILSTM_BERT, self).__init__(**kwargs)
    ...
    def build(self, bias=0):
         ...
         # encode parent using a LSTM
        parent_input = tf.keras.layers.Input(shape=(self.max_length,), name="parent_input")
        ...
        parent_rnn = Bidirectional(LSTM(128, return_sequences=False))(parent_emb)

        print("RNN size :",parent_rnn.shape)
        ...
        #concatenate parent and target embeddings
        x = concatenate([parent_rnn, bert_output])

        print(x.shape, "sizee")

        dense = tf.keras.layers.Dense(128, activation='tanh')(x)
        pred = tf.keras.layers.Dense(1, activation=None, bias_initializer=tf.keras.initializers.Constant(bias))(dense)
        self.model = tf.keras.models.Model(inputs=[parent_input] + bert_inputs, outputs=pred)
        self.model.compile(loss=self.loss,
                      optimizer=tf.keras.optimizers.Adam(learning_rate=self.lr),
                      metrics=METRICS)
        if self.show_summary:
    ...
    def fit(self, train, dev, pretrained_embeddings, bert_weights=None, batch_size=32):
        #prepare vocab
        ...
        self.history = self.model.fit([parent_input] + list(train_input),
                                      train_targets,
                                      validation_data=([parent_dev_input] + list(dev_input), dev_targets),
                                      epochs=self.epochs,
                                      callbacks=[self.earlystop],
    ...
    def predict(self, val_pd):
        val_input, val_labels = self.to_bert_input(val_pd, text_to_encode = "text")
        parent_val_input = self.text_process(val_pd.parent)
        predictions = self.model.predict([parent_val_input] + list(val_input))
        print('Stopped epoch: ', self.earlystop.stopped_epoch)
        if self.save_predictions:
